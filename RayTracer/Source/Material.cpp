...
bool Dielectric::Scatter(const ray_t& ray, const raycastHit_t& raycasHit, color3_t& attenuation, ray_t& scattered)
{
    ...
    if (glm::dot(ray.direction, raycasHit.normal) < 0)
    {
        ...
        cosine = -glm::dot(ray.direction, raycasHit.normal) / ray.direction.length();
    }
    else
    {
        // hitting from inside
        ...
        cosine = m_refractiveIndex * glm::dot(ray.direction, raycasHit.normal) / ray.direction.length();
    }
